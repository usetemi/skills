...
1. Should the migration (A) backfill existing rows in place, or (B) leave them null and only populate going forward?
   My prediction: backfill existing rows in place.
```

### 3. Display each question before its prediction

For each question, display the question first, then a short, visible prediction on a separate line labeled `My prediction:`. In interactive tools, put both lines in the question text, with answer choices following. Do not put predictions in an earlier commentary message. In a batch, keep each question immediately followed by its own prediction. Do not rely on hidden reasoning to preserve the calibration loop across models, tools, or context compaction.
